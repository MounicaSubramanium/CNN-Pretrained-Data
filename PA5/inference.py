...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            counter[self.getJailPosition()] = 1.0
            ...
            for position in self.legalPositions:
                ...
                counter[position] = emissionModel[distance] * self.beliefs[position]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # for every old position in list of legal positions
        for oldPos in self.legalPositions:

            # get the new position distribution of the ghost
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))

            # for ever =y pair of new position and probsbility in new position distribution
            for newPos, prob in newPosDist.items():
                # compute counter value
                counter[newPos] += prob * self.beliefs[oldPos]

        # set counter value to beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # initialize summation varaible to 0
        summation = 0

        # initialize particles list
        self.particles = []

        # while sum is less than number of particles
        while summation < self.numParticles:

            # for every position in legal positions list
            for position in self.legalPositions:

                # if sum is again less than number of particles
                if summation < self.numParticles:
                    # append position of the particle to the particles list
    ...
    def observe(self, observation, gameState):
        ...
        # Case 1
        # if the noisy distance is none
        if noisyDistance == None:

            # compute value for particles distance list
            self.particles = [self.getJailPosition()] * self.numParticles


        else:

            # set values for all possible and old belief variables
            allPossible, oldBelief = util.Counter(), self.getBeliefDistribution()

            # for every location in legal positions
            for location in self.legalPositions:

                # calculate the manhattan distance for the current location and pacman position
                distance = util.manhattanDistance(location, pacmanPosition)

                # all position location is calculated using emission model distance and old belief location
                allPossible[location] += emissionModel[distance] * oldBelief[location]

            # case 2
            # if any all possible values are not found
            if not any(allPossible.values()):

                # initialize the game state uniformly
                self.initializeUniformly(gameState)
                ...
                # for underscore in range of number of particles
                for _ in range(0, self.numParticles):

                    # append all position variables to the temporary variable
                    temp.append(util.sample(allPossible)) #recreate samples based on distribution allPossible

                # set temp value to particles
                self.particles = temp
    ...
    def elapseTime(self, gameState):
        ...
        # for every old position in paricles position list
        for oldPos in self.particles:

            # get new position of the particle (ghost)
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))

            # apprend new position value to the temporary variable
    ...
    def getBeliefDistribution(self):
        ...
        # for every particle in particles list
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # create a list of particle's legal positions
        ...
        # while sum is less than the number of particles
        while summ < self.numParticles:

            # for every position in the randomly shuffled particle postion list
            for position in possPositions:

                # if sum is again less than the number of particles
                if summ < self.numParticles:

                    # append current position to the particles list
                    self.particles.append(position)
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for index, particle in enumerate(self.particles):
            ...
            # for ever i in range of number of ghosts
            for i in range(self.numGhosts):

                # if distance is equal to i
                if noisyDistances[i] == None:

                    # get the particle distance
                    particle = self.getParticleWithGhostInJail(particle, i)

                else:

                    # get the manhattan distance of the particle from pacman position
                    distance = util.manhattanDistance(particle[i], pacmanPosition)

                    # calculate temporary variable value
                    temp *= emissionModels[i][distance]

            # update all pos counter variable
            allPossible[particle] += temp

        # if any value is not in all pos list
        if not any(allPossible.values()):


            self.initializeParticles()

            # for every index particle in enumerated particles
            for index, particle in enumerate(self.particles):

                # for every i in range of number of ghosts
                for i in range(self.numGhosts):

                    # set the noisy distance to none
                    if noisyDistances[i] == None:
            ...
            # normalize all pos value
            allPossible.normalize()

            # initialize temp variable
            temp = []

            # for every underscore in range of number of particles
            for _ in range(0, self.numParticles):

                # append all pos value to temporary variable
                temp.append(util.sample(allPossible))

            # set particles to temp
            self.particles = temp
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                # get new position distribution for the ghost
    ...
    def getBeliefDistribution(self):
        ...
        # for every particle in list of particles
        for particle in self.particles:

            # increment counter by 1
            counter[particle] += 1.0

        # normalize counter and return
        counter.normalize()
        return counter
